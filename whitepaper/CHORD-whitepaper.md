...
9. **$B_{\mathrm{LCB}}$ now earns its complexity, and is reproducible** (§4.2, validated in C.5).
   ...
   *untested* one-sided content (a firehose post regresses to neutral, not below). That last gap
   ...
   The author lift is deliberately **asymmetric and quality-based** (`hierarchical_prior_quality`,
   the default when E9 is on): a poor *approval* record can only *lower* the prior — preserving the
   firehose pre-emption — but *raising* it above the cluster baseline is licensed **only by the
   author's earned cross-cluster vouches** (the §10 merit channel), by a credit
   $\max(0,\bar v_{ac})\cdot w/(w+n_0)$. This is the fix for the failure mode the re-validation
   surfaced (§13.11): an *approval*-history prior props up exactly the broadly-approved-but-shallow
   **bait** and partisan-consistent authors that the rest of CHORD targets — routing the lift
   through the un-forgeable vouch channel instead credits demonstrated *merit*, not crowd-pleasing.
   ...
   *lowers* author extremity rather than raising it — no longer propping up the bait or the
   ...
   refinements to graduate — with one honest caveat, now itself tested: because the lift flows
   through the vouch channel, E9 leans on *that* channel's collusion defenses (§5 out-diversity
   / §13.10 loyalty). A ring that *forges* vouches (modeled by `ring_forge_vouches`) finds no
   new lever — its single-target puppets carry ~0 out-diversity λ, so the forged merit votes are
   discounted, and a low-quality target's honest anti-vouches dilute the rest; forging buys the
   target no reach (§13.11). The caveat is real but the existing machinery already covers it.
...
|---|---|---|
| **Hierarchical prior** (E9, `hierarchical_prior`, §13#9) — **now on by default** | $B_{\mathrm{LCB}}$ predicts-low on an *untested* one-sided firehose before the budget bites; +28\% delivered value | *Originally* on **approval history**, which also propped up a broadly-approved-but-shallow **bait**, a **ring target**, and partisan consistency — *raising* author extremity. **Resolved** by rebasing the lift onto the **quality (vouch) channel** (`hierarchical_prior_quality`, default): approval can only *lower* the prior, and raising it requires earned vouches. This kept the firehose win while *lowering* extremity (Δ −0.02 vs the approval basis's +0.16) and no longer propping the bait/ring, so E9 **graduated to a default** after its own re-validation pass. New standing cost: the lift now depends on the **vouch channel's** collusion defenses. That cost was then *tested* — the simulator's ring can forge vouches (`ring_forge_vouches`), and forging buys the target no reach: single-target puppets carry ~0 out-diversity λ so their forged votes are discounted, and a low-quality target's honest anti-vouches dilute the rest. The existing out-diversity/loyalty machinery already carries the extra weight. (The other ring tests still isolate E9, since the sim's *honest*-vouching puppets would otherwise credit a genuine-quality target's real merit and confound the approval-defense measurement.) |
