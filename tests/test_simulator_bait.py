...
on raises delivered true value and collapses the bait author's reach.
"""
import numpy as np
import pytest
...
def test_depth_handling_raises_delivered_true_value(onoff):
    off_tv, _ = onoff["off"]
    on_tv, _ = onoff["on"]
    assert on_tv > off_tv + 0.03, (
        f"depth handling should raise delivered true value ({off_tv:.4f} -> {on_tv:.4f})"
    )
...
def test_depth_handling_demotes_the_bait_author(onoff):
    ...
    _, on_bait = onoff["on"]
    assert on_bait < 0.6 * off_bait, (
        f"depth handling should collapse the bait author's reach "
